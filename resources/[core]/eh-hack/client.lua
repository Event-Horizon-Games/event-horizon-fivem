---@diagnostic disable: redefined-local, cast-local-type, need-check-nil, param-type-mismatch
...
function StartHack(life, time)
	if life and time then

		if life <= 0 or life > 6 then
			Citizen.Trace('Invalid lives passed')
			return -1
		end

		if time < minTime or time > maxTime then
			Citizen.Trace('Invalid time passed')
			return -1
		end
		...
		if not timer or not availableLife then
			Citizen.Trace('Parameters must be numbers')
			return -1
		end
		...
		if isGameActive then
			Citizen.Trace('Game is already ongoing.')
			return -1
		end

		isGameActive = true
		local gameReturn = StartGame()

		-- TODO better way to handle the output
		if gameReturn == 1 then
			Citizen.Trace('Hack successful!')
			SendNUIMessage({type  = 'success'})
			return 1
		elseif gameReturn == 2 then
			Citizen.Trace('Ran out of time!')
			SendNUIMessage({type  = 'fail'})
			return 2
		elseif gameReturn == 0 then
			Citizen.Trace('Ran out of lives!')
			SendNUIMessage({type  = 'fail'})
			return 0
		else
			Citizen.Trace('Error occured in the game.')
			SendNUIMessage({type  = 'fail'})
			return -1
		end
		...
		Citizen.Trace('Invalid parameters passed')
		return -1
	end
end

exports("HackMinigame", StartHack)

RegisterCommand('testhack', function(source, args)
	local lives = tonumber(args[1])
	local time = tonumber(args[2])

	StartHack(lives, time)
end)
